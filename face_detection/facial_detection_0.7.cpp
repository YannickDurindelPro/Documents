// CPP program to detect face, eyes, ear, and mouth in a video
...
void detectAndDraw(Mat& img, CascadeClassifier& faceCascade, CascadeClassifier& eyeCascade, CascadeClassifier& mouthCascade, CascadeClassifier& leftEarCascade, CascadeClassifier& rightEarCascade, double scale);
string cascadeName, eyeCascadeName, mouthCascadeName, leftEarCascadeName, rightEarCascadeName;
...
int main(int argc, const char** argv)
{
    ...
    CascadeClassifier faceCascade, eyeCascade, mouthCascade, leftEarCascade, rightEarCascade;
    double scale = 1;
    ...
    rightEarCascade.load("/usr/include/opencv4/opencv2/haarcascades/haarcascades_mcs_rightear.xml");
    ...
    if (capture.isOpened())
    {
        ...
        while (1)
        {
            ...
            detectAndDraw(frame1, faceCascade, eyeCascade, mouthCascade, leftEarCascade, rightEarCascade, scale);
...
void detectAndDraw(Mat& img, CascadeClassifier& faceCascade, CascadeClassifier& eyeCascade, CascadeClassifier& mouthCascade, CascadeClassifier& leftEarCascade, CascadeClassifier& rightEarCascade, double scale)
{
    ...
    for (size_t i = 0; i < faces.size(); i++)
    {
        ...
        // Detection of right ear in the input image
        vector<Rect> rightEarObjects;
        rightEarCascade.detectMultiScale(smallImgROI, rightEarObjects, 1.3, 5, 0 | CASCADE_SCALE_IMAGE, Size(30, 30));

        // Draw rectangles around right ear
        for (size_t j = 0; j < rightEarObjects.size(); j++)
        {
            Rect nr = rightEarObjects[j];
            Rect rightEarRect(cvRound((r.x + nr.x) * scale), cvRound((r.y + nr.y) * scale), cvRound(nr.width * scale), cvRound(nr.height * scale));
            rectangle(img, rightEarRect, Scalar(120, 120, 120), 3, 8, 0);
            //imwrite("rightEar.jpg", img);
        }
