...
void detectAndDraw(Mat& img, CascadeClassifier& faceCascade, CascadeClassifier& eyeCascade, CascadeClassifier& mouthCascade, double scale);
string cascadeName, eyeCascadeName, mouthCascadeName;
...
int main(int argc, const char** argv)
{
    ...
    if (capture.isOpened())
    {
        ...
        cout << "Face, Eye, and Mouth Detection Started...." << endl;
        while (1)
        {
            capture >> frame;
            if (frame.empty())
            ...
            detectAndDraw(frame1, faceCascade, eyeCascade, mouthCascade, scale);
...
void detectAndDraw(Mat& img, CascadeClassifier& faceCascade, CascadeClassifier& eyeCascade, CascadeClassifier& mouthCascade, double scale)
{
    ...
    for (size_t i = 0; i < faces.size(); i++)
    {
        ...
        eyeCascade.detectMultiScale(smallImgROI, nestedObjects, 1.05, 2, 0 | CASCADE_SCALE_IMAGE, Size(30, 30));
